<template>
	<div class="app-container">
		<el-tabs v-model="activeName" @tab-click="handleClick">
			<el-tab-pane label="个人费用" name="first">
				<el-row>
					...
					<el-col :span="18">
						...
						<el-table :key="tableKey" v-loading="checkinListLoading" :data="checkinList" border fit highlight-current-row
						 style="width: 100%;" :header-cell-style="{backgroundColor: '#F5F7FD', color: '#3B3B3B'}">
							...
							<el-table-column label="费用时间" min-width="110px" align="center">
								<template slot-scope="scope">
									{{ scope.row.startDate }}
								</template>
							</el-table-column>
							<el-table-column label="执行时间" min-width="110px" align="center">
								<template slot-scope="scope">
							...
							<el-table-column label="备注" min-width="140px" align="center">
								<template slot-scope="scope">
									{{ scope.row.remarks }}
								</template>
							</el-table-column>
			...
			<el-tab-pane label="费用查询" name="second">
				...
				<el-table :key="tableKey" v-loading="listLoading" :data="list" border fit highlight-current-row style="width: 100%;"
				 :header-cell-style="{backgroundColor: '#F5F7FD', color: '#3B3B3B'}">
					...
					<el-table-column label="床位" min-width="130px" align="center">
						<template slot-scope="scope">
							{{ scope.row.buildingName + scope.row.floorName + scope.row.bedName }}
						</template>
					</el-table-column>
					<el-table-column label="护理等级" min-width="110px" align="center">
						<template slot-scope="scope">
					...
					<el-table-column label="退费时间" min-width="200px" align="center">
						<template slot-scope="scope">
							{{ scope.row.createTime }}
						</template>
					</el-table-column>
	...
	export default {
		...
		methods: {
			...
			fetchCheckinList() {
				...
				findByCheckinNo(this.checkinQuery).then(response => {
					if (response.data) {
			...
			fetchList() {
				...
				findDetailPage(this.listQuery).then(response => {
